class Customer::ChildrenController < ApplicationController
  ...
  def create
    ...
    if @child.save
      ...
      redirect_to new_family_path(@facility)
    else
  ...
  def child_params
    ...
    :breastfeeding_amount, :situation_after_breastfeeding, :amount_of_milk_after_baby_food, :baby_food_progress, :appetite, :favorite_food, :hated_food,
    :how_to_eat, :meal_time, :how_to_lay_it_down, :falling_asleep, :waking_up, :sleeping_position, :is_deep_sleep, :habits_when_sleeping, :diaper_usage,
    :is_sensitive_to_rash, :urination, :defecation, :number_of_defecation, :favorite_game, :favorite_toy, :hospital_name, :hospital_telephone_number,
    :memo, :is_active)
  end
